...
char * get_db_module_path(char * filename, char * module) {
	...
	if (filename == NULL || module == NULL) {
		a6o_log(A6O_LOG_MODULE, A6O_LOG_LEVEL_ERROR,"[-] Error :: get_db_module_path :: Invalids parameters\n");
		return NULL;
	}

	dbdir = a6o_std_path(A6O_LOCATION_BASES);
	if (dbdir == NULL) {
		a6o_log(A6O_LOG_MODULE, A6O_LOG_LEVEL_ERROR,"[-] Error :: get_db_module_path :: Can't get database directory!\n");
		return NULL;
	}
